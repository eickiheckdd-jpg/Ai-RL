...
public class MLP {
    private final float[][] weights1, weights2, weights3;
    private final float[] bias1, bias2, bias3;
    private final float[] hidden1, hidden2;
    ...
    private void computeLayer(float[] in, float[][] w, float[] b, float[] out, boolean relu) {
        for (int j = 0; j < out.length; j++) {
            ...
            out[j] = relu ? Math.max(0, sum) : sum; // ReLU or Linear
        }
    }
    
    // Adam optimizer backpropagation arrays would be initialized here...
}
