...
public final class Newgen6Client implements ClientModInitializer {
    ...
    private static UUID targetId = null;

    private static PlayerEntity getTrackedEnemy(ClientPlayerEntity player) {
        if (targetId != null) {
            for (PlayerEntity p : player.getEntityWorld().getPlayers()) {
                if (p.getUuid().equals(targetId)) return p; // same entity as before, alive or dead
            }
            targetId = null; // truly gone (disconnected/out of range) - release lock so we can acquire a new target next call
            return null; // NOT a kill, just an untracked disappearance
        }
        PlayerEntity next = findNearestOpponent(player);
        if (next != null) targetId = next.getUuid();
        return next;
    }
    ...
    public void onInitializeClient() {
        ...
        HudRenderCallback.EVENT.register((drawContext, tickDelta) -> {
            ...
            int y = 10;
            drawContext.drawText(client.textRenderer, "Newgen6 A2C Engine", 10, y, 0xFF00FF00, false); y += 12;
            drawContext.drawText(client.textRenderer, "Mode: " + (enabled ? "ACTIVE" : "IDLE"), 10, y, enabled ? 0xFF00FF00 : 0xFFFF0000, false); y += 12;
            drawContext.drawText(client.textRenderer, "Current Action: " + currentActionName, 10, y, 0xFFFFFFFF, false); y += 12;
            drawContext.drawText(client.textRenderer, String.format("Moving Avg Reward: %.3f", avgReward), 10, y, 0xFFFFFFFF, false); y += 12;
            drawContext.drawText(client.textRenderer, String.format("A2C Advantage (Loss): %.3f", currentLoss), 10, y, 0xFFFFFFFF, false); y += 12;
            drawContext.drawText(client.textRenderer, "Total Steps: " + actionsTaken, 10, y, 0xFFAAAAAA, false);
        });
    }
    ...
    private static void registerBaseActions() {
        ...
        ACTION_REGISTRY.add(new AIAction("Hold", client -> {
            // Intentionally does nothing - lets the agent keep its current aim if it's already good.
        }, client -> true));
    ...
    private static void tick(MinecraftClient client) {
        ...
        PlayerEntity enemy = getTrackedEnemy(client.player);
        if (client.player.isDead() || (enemy != null && enemy.isDead())) {
            ...
            if (previousObservation != null) {
                ...
                if (terminalState != null) {
                    ...
                    POLICY.train(previousObservation.values, previousAction, terminalReward, vNext, true, getActionMasks(client));
                }
            }
            resetSession();
            return;
    ...
    private static void runAI(MinecraftClient client, Observation current) {
        ...
        if (previousObservation != null) {
            ...
            POLICY.train(oldStateArray, oldAction, reward, vNext, false, masks);
            actionsTaken++;
        }
    ...
    private static double calculateUnifiedReward(Observation oldState, Observation newState) {
        ...
        if (currentAimAlignment > 0.98) {
            reward += 0.5; // strong bonus for near-perfect aim
        } else {
            reward += (currentAimAlignment - 1.0) * 1.5; // scaling penalty: the further off-target, the worse
        }
    ...
    private static final class NeuralPolicy {
        private static final double ENTROPY_BETA = 0.02; // higher = more exploration pressure, prevents collapsing onto 1-2 actions
        ...
        int chooseAction(double[] x, boolean[] masks, double temperature) {
            ...
            for (int i = 0; i < p.length; i++) {
                if (Double.isNaN(p[i])) break; // corrupted output - stop trying to sample from it
                cum += p[i];
                if (r <= cum) return i;
            }
            // Fallback: pick uniformly among valid actions instead of always defaulting to index 0
            List<Integer> valid = new ArrayList<>();
            for (int i = 0; i < numActions; i++) if (masks[i]) valid.add(i);
        ...
        void train(double[] x, int action, double reward, double vNext, boolean terminal, boolean[] masks) {
            ...
            double advantage = clampRange(target - f.value, -20.0, 20.0);
            currentLoss = advantage;
            
            double entropy = 0.0;
            for (int i = 0; i < numActions; i++) if (masks[i]) entropy -= p[i] * Math.log(p[i] + 1e-8);

            double[] dLogits = new double[numActions];
            for (int i = 0; i < numActions; i++) {
                if (!masks[i]) continue;
                dLogits[i] = clampRange(p[i] * advantage, -10.0, 10.0);
                dLogits[i] += ENTROPY_BETA * p[i] * (Math.log(p[i] + 1e-8) + entropy); // push toward higher entropy = more exploration
            }
            dLogits[action] -= advantage;
            double dValue = clampRange(-advantage, -10.0, 10.0);
        ...
        private double clampRange(double v, double lo, double hi) { return Math.max(lo, Math.min(hi, v)); }
    }
    ...
    private static void loadModel() {
        ...
        try (DataInputStream in = new DataInputStream(new BufferedInputStream(Files.newInputStream(MODEL_FILE)))) {
            if (in.readInt() != 4) throw new IOException("Old model version.");
