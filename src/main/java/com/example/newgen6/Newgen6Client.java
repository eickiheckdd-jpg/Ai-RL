...
private static final NeuralPolicy POLICY =
    ...
    private static double calculateAiReward(
        ...
        double beforeYaw = before.values[last + 12] * 180.0;
        double beforePitch = before.values[last + 13] * 90.0;
        double afterYaw = after.values[last + 12] * 180.0;
        double afterPitch = after.values[last + 13] * 90.0;

        double beforeError =
                Math.hypot(beforeYaw, beforePitch);

        double afterError =
                Math.hypot(afterYaw, afterPitch);

        /*
         * Positive when the selected action made the aim better.
         */
        double reward =
                (beforeError - afterError) / 30.0;

        /*
         * Small accuracy bonuses provide a useful dense signal.
         */
        if (afterError < 10.0) reward += 0.03;
        if (afterError < 3.0) reward += 0.08;

        /*
         * Don't punish exploration heavily.
         */
        reward -= 0.001;
